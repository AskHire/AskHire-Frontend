...
export default function UserRoles() {
 ...
 const handleSaveRole = async (userId) => {
      ...
      try {
        await fetch(`https://localhost:7256/api/users/${userId}`, {
          method: "PUT",
          headers: {
                  ...
                  {editingRoleUserId === user.userId ? (
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        onClick={() => handleSaveRole(user.userId)}> Save
                      </button>
                      
                      <button
                        className="text-sm text-red-500 underline"
                        onClick={() => setEditingRoleUserId(null)}> Cancel
                      </button>
                    </div>
                    ) : (
                      ...
                      <button
                        ...
                        user.role === "Candidate" ? "bg-yellow-500" :
                                      "bg-blue-500"}`}
                        onClick={() => setEditingRoleUserId(user.userId)}>
                        {user.role}
                      </button>
                  )}
                </div>
                      ...
                      <h2 className="text-2xl font-bold text-center text-gray-700">User Details</h2>
                        ...
                        <div className="flex justify-end mt-6">
                          <button
                            ...
                            onClick={() => setSelectedUser(null)}> Close
                          </button>
                        </div>
                    </div>
                  </div>
